...
interface GalleryListProps {
    mediaData: DataInterface[]
    hasMoreInitially: boolean
}

export default function MediaGrid({ mediaData, hasMoreInitially }: GalleryListProps) {
    const [updatedData, setUpdatedData] = useState(mediaData)
        ...
        <>
            <div className="gallery_listing">
                {updatedData.map((item, index) => (
                    ...
                    >
                        <Image
                            src={item?.image ? item.image : `/images/placeholders/hapennig_gallery_placeholder.webp`}
                            alt={item.title}
                            width={760}
                            height={600}
                            loading="lazy"
